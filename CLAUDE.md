...
Server only:
```bash
cd server && npm run dev   # nodemon server/src/index.js
```
...
│   ├── templates/     HTML templates rendered by Puppeteer for PDF output
...
Settlement (`thanhLy.service.js`) and cancellation (`huyHopDong.service.js`) both trigger room status transitions (`cho_thue → trong`) and access `HoaDon` to calculate outstanding debt.
...
Managed via `.env` (never committed). Key vars:
